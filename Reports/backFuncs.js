  ...
  switch (type) {
    case "messages": {
      if (from == undefined && to == undefined) {
        ...
        return fileDetails["process"].reduce(
          (total, current) =>
            (total +=
              current["date"] >= from &&
              current["date"] <= to &&
              rulesSet.has(current["rule"])
                ? 1
                : 0),
          0
        );
    ...
    case "Error Detection": {
      if (from === undefined && to === undefined) {
        return fileDetails["process"].reduce(
          (total, current) =>
            (total += current["rule"] === "Error Detection" ? 1 : 0),
          0
        );
      } else {
        return fileDetails["process"].reduce(
    ...
    case "high": {
      if (from == undefined && to == undefined) {
        return fileDetails["process"].reduce(
          (total, current) => (total += current["rank"] == 3 ? 1 : 0),
          0
        );
      } else {
        return fileDetails["process"].reduce(
...
exports.messagesFilterBaseOnRule = function (fileDetails, from, to, rules) {
  ...
  if (rules == undefined) {
    fileDetails["process"].forEach((data) => {
      rulesCounters[`${data["rule"]}`] =
        rulesCounters[`${data["rule"]}`] == undefined
          ? 1
          : rulesCounters[`${data["rule"]}`] + 1;
    });
  } else {
    for (let i = 0; i < fileDetails["process"].length; i++) {
      let data = fileDetails["process"][i];
      if (
        rulesSet.has(data["rule"]) &&
        data["date"] >= from &&
        data["date"] <= to
      ) {
        rulesCounters[`${data["rule"]}`] =
  ...
  const arr = Object.keys(rulesCounters).map((name) => ({
    ...
    name: name,
    rate: ((rulesCounters[name] / fileDetails["process"].length) * 100).toFixed(
      2
  ...
  return [
    {
      y: rankCounters[0],
      name: "Low",
      rate: ((rankCounters[0] / sum) * 100).toFixed(2),
    },
    {
      y: rankCounters[1],
      name: "Medium",
      rate: ((rankCounters[1] / sum) * 100).toFixed(2),
    },
    {
      y: rankCounters[2],
      name: "High",
      rate: ((rankCounters[2] / sum) * 100).toFixed(2),
    },
  ];
};
exports.divideMessagesByXMin = function (
  fileDetails,
  minutes,
  from,
  to,
  ...
  minutesBetweenFromTo =
    minutesBetweenFromTo / Math.trunc(minutesBetweenFromTo) - 1 > 0
      ? Math.trunc(minutesBetweenFromTo) + 1
      : Math.trunc(minutesBetweenFromTo);
  ...
  for (let i = 0; i < fileDetails["process"].length; i++) {
    ...
    if (rulesSet.has(data["rule"]) && dateOfData >= from && dateOfData <= to) {
      arr[
        Math.trunc((Date.parse(data["date"]) - from) / (1000 * 60) / minutes)
      ]++;
    }
  }
...
exports.divideRankByXMin = function (
  fileDetails,
  minutes,
  rank,
  from,
  ...
  for (let i = 0; i < fileDetails["process"].length; i++) {
    ...
    if (
      data["rank"] === rank &&
      (rules === undefined ||
        (rulesSet.has(data["rule"]) && dateOfData >= from && dateOfData <= to))
    ) {
      arr[
        Math.trunc((Date.parse(data["date"]) - from) / (1000 * 60) / minutes)
      ]++;
