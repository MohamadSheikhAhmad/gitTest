...
const FolderLink =
  "https://drive.google.com/drive/folders/1LzZPQsyF2w3qpqgwoe8nc017rDQ49I49";
const LOCAL_FOLDER_PATH = "./driveLogs"; // change this to the local folder that we save the files locally
...
oAuth2Client.setCredentials({
  access_token:
    "ya29.a0AfB_byAqQ07NIw6tnrv7IghyKxOgZWkDYziG3-0ImG7pMZ-nPbUhO9q0WwmKJarqeh_JxGmKujrbe5eVyGFbWRQWamxjRiGp7GNcrGiP86VWbSebACF_NwLybrsuy1NwQZuoKPN8qtorxzwrUYvGyAru0PcscE1YjQaCgYKAT8SARESFQGOcNnC88aDn5i-0aUQ8O8FWznX2Q0169",
  refresh_token:
    "1//03x8yenC4Qx8uCgYIARAAGAMSNwF-L9IrT_OzOaJYPcRL4C4AZuTW-5hY9-3kF_Z4JoF3H8sas-fufwgG06noByvrMbwGAF7Vphc",
});
...
async function downloadAndSaveFile(file, databaseName) {
  ...
  if (fileName.includes(".txt")) {
    response.data
      .on("end", () => {
        ...
        checkCollection(fileName, databaseName);
...
function pollFile(logDriveFolder, databaseName) {
  const drive = google.drive({ version: "v3", auth: oAuth2Client });
    ...
    drive.files.list(
      ...
      (err, res) => {
        ...
        const changedFiles = fileList.filter((file) => {
          const previousFile = previousFileList.find(
            (prevFile) => prevFile.id === file.id
          );
          return (
            !previousFile || previousFile.modifiedTime !== file.modifiedTime
          );
        });
        ...
        if (changedFiles.length > 0) {
          changedFiles.forEach(async (file) => {
            ...
            await downloadAndSaveFile(file, databaseName);
          });
...
async function checkCollection(fileName, databaseName) {
  ...
  try {
    ...
    const dbconn = await getMongooseConnection(databaseName);
    if (collectionExists) {
      try {
        await dbconn.connection.db.collection(collectionNameToCheck).drop();
        console.log(
          `Collection ${collectionNameToCheck} dropped successfully.`
        );
      } catch (error) {
        console.error(
          `Error dropping collection ${collectionNameToCheck}:`,
          error
        );
      }
    }
